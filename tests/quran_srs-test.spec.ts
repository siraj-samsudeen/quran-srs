...
test('single_entry', async ({ page }) => {
  ...
  await expect(page.locator('h1')).toContainText('3 - S2 Al Baqara Page 2 - إِنَّ الَّذِينَ كَفَرُوا');
  ...
  await expect(page).toHaveURL("http://localhost:5001/");
  await expect(page.locator('#main')).toContainText('2 - S2 Al Baqara Start');
});
...
test('bulk_entry', async ({ page }) => {
  ...
  await expect(page.locator('h1')).toContainText('3 - S2 Al Baqara Page 2 => 7 - S2 Al Baqara');
  await expect(page.getByRole('textbox', { name: 'Revision Date' })).toHaveValue(currentDate); 
  await page.locator('uk-select[name="mode_id"] >> button.uk-input-fake').first().click();
  ...
  await page.getByRole('spinbutton', { name: 'Plan Id' }).fill('1');
  await page.getByRole('button', { name: 'Save' }).click();
  await expect(page.getByRole('heading', { name: '8 - S2 Al Baqara => 12 - S2' })).toBeVisible();
...
test('revision_bulk_update', async ({ page }) => {
  ...
  await page.getByRole('spinbutton', { name: 'Plan Id' }).click();
  await page.getByRole('spinbutton', { name: 'Plan Id' }).fill('1');
  await expect(page.locator('h1')).toContainText('99 - S4 An Nisa => 100 - S4 An Nisa');
  await expect(page.getByRole('textbox', { name: 'Revision Date' })).toHaveValue(currentDate); //TODO Add current date
  await page.locator('uk-select[name="mode_id"] >> button.uk-input-fake').first().click();
...
test('bulk_edit_revision_range', async ({ page }) => {
 ...
 await page.getByRole('listitem').filter({ hasText: 'SEQ' }).locator('a').click();
 await page.getByRole('button', { name: 'Save' }).click();
 await expect(page.getByRole('heading', { name: '38 - S2 Al Baqara => 42 - Juz' })).toBeVisible();
 await page.getByRole('button', { name: 'Cancel' }).click();
 await expect(page).toHaveURL("http://localhost:5001/");
//  bulk edit from revision range
 await page.getByRole('link', { name: '- S2 Al Baqara -> 37 - S2 Al Baqara' }).click();
 await page.getByRole('button', { name: 'Cancel' }).click();
 await expect(page).toHaveURL("http://localhost:5001/");
 await page.getByRole('link', { name: '- S2 Al Baqara -> 37 - S2 Al Baqara' }).click();
...
test('continue_with_bulk_add', async ({ page }) => {
  ...
  await expect(page.getByText('56 - S3 Aal-e-Imran Page 5')).toBeVisible();
  await page.getByRole('link', { name: '56 - S3 Aal-e-Imran Page 5' }).click();
  await page.getByRole('spinbutton', { name: 'Plan ID' }).click();
  await page.getByRole('spinbutton', { name: 'Plan ID' }).fill('1');
  await page.getByRole('button', { name: 'Save' }).click();
...
test('bulk_add_with_custom_range', async ({ page }) => {
  ...
  await page.getByRole('spinbutton', { name: 'Plan ID' }).click();
  await page.getByRole('spinbutton', { name: 'Plan ID' }).fill('1');
  await expect(page.getByRole('heading', { name: '155 - S7 Al A\'raf Page 5 => 164 - S7 Al A\'raf' })).toBeVisible();
  await page.getByRole('button', { name: 'Save' }).click();
  expect(page.url()).toContain("http://localhost:5001/revision/bulk_add?page=");
  await expect(page.getByRole('heading', { name: '165 - S7 Al A\'raf => 174 - S7' })).toBeVisible();
  await page.getByRole('button', { name: 'Cancel' }).click();
  await expect(page).toHaveURL("http://localhost:5001/");
  await expect(page.getByText('155 - S7 Al A\'raf Page 5').first()).toBeVisible();
});
...
test('single_add_with_custom_range', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '255 - S14 Ibrahim Start' })).toBeVisible();
  await page.getByRole('button', { name: 'Save' }).click();
  expect(page.url()).toContain("http://localhost:5001/revision/add?page=");
  await expect(page.getByRole('heading', { name: '256 - S14 Ibrahim Page 2' })).toBeVisible();
  await page.getByRole('button', { name: 'Cancel' }).click();
  await expect(page).toHaveURL("http://localhost:5001/");
  await expect(page.getByText('255 - S14 Ibrahim Start').first()).toBeVisible();
});
...
test('shift_selection', async ({ page }) => {  
  ...
  await page.getByRole('button', { name: 'Save' }).click();
  await page.getByRole('button', { name: 'Cancel' }).click();
  await page.getByRole('link', { name: '- S24 Nur -> 359 - S25 Furqan Start' }).click();
...
test('started_word_of_the_page', async ({ page }) => {  
  ...
  await page.getByRole('textbox', { name: 'page' }).fill('2');
  await page.getByRole('button', { name: 'Single Entry' }).click();
  await expect(page.locator('h1')).toContainText('2 - S2 Al Baqara Start - الم ذَلِكَ الْكِتَابُ');
  await page.getByRole('button', { name: 'Cancel' }).click();
  await page.getByRole('textbox', { name: 'page' }).click();
  await page.getByRole('textbox', { name: 'page' }).fill('150');
  await page.getByRole('button', { name: 'Single Entry' }).click();
  await expect(page.locator('h1')).toContainText('150 - S6 Al An\'am End - هَلْ يَنْظُرُونَ إِلَّا');
  await page.getByRole('button', { name: 'Cancel' }).click();
  await page.getByRole('textbox', { name: 'page' }).click();
  await page.getByRole('textbox', { name: 'page' }).fill('300');
  await page.getByRole('button', { name: 'Single Entry' }).click();
  await expect(page.locator('h1')).toContainText('300 - S18 Kahf - وَلَقَدْ صَرَّفْنَا فِي');
  await page.getByRole('button', { name: 'Cancel' }).click();
  await page.getByRole('textbox', { name: 'page' }).click();
  await page.getByRole('textbox', { name: 'page' }).fill('600');
  await page.getByRole('button', { name: 'Single Entry' }).click();
  await expect(page.locator('h1')).toContainText('600 - S101 Qari\'ah Start - إِنَّ الْإِنْسَانَ لِرَبِّهِ');
  await page.getByRole('button', { name: 'Cancel' }).click();
  await page.getByRole('textbox', { name: 'page' }).click();
  await page.getByRole('textbox', { name: 'page' }).fill('604');
  await page.getByRole('button', { name: 'Single Entry' }).click();
  await expect(page.locator('h1')).toContainText('604 - S112 Ikhlas Start - قُلْ هُوَ اللَّهُ');
  await page.getByRole('button', { name: 'Cancel' }).click();
  await page.getByRole('textbox', { name: 'page' }).click();
  ...
  await page.getByRole('button', { name: 'Bulk Entry' }).click();
  await expect(page.locator('h1')).toContainText('599 - S99 Zalzalah Start => 603 - S109 Kafirun Start');
...
test('page_field_fill_with_last_added_page_no', async ({ page }) => {  
  ...
  await page.getByRole('button', { name: 'Single Entry' }).click();
  await expect(page.locator('h1')).toContainText('410 - S30 Rum End - وَلَئِنْ أَرْسَلْنَا رِيحًا');
  await expect(page.getByRole('spinbutton', { name: 'Page' })).toHaveValue('410');
  await page.getByRole('button', { name: 'Save' }).click();
  await expect(page.locator('h1')).toContainText('411 - S31 Luqman Start - الم تِلْكَ آيَاتُ');
  ...
  // after bulk entry
  await page.getByRole('button', { name: 'Bulk Entry' }).click();
  await expect(page.getByRole('heading', { name: '411 - S31 Luqman Start => 415' })).toBeVisible();
  await page.getByRole('button', { name: 'Save' }).click();
  await page.getByRole('button', { name: 'Cancel' }).click();
...
test('page_exceed', async ({ page }) => {  
  ...
  await page.getByRole('button', { name: 'Bulk Entry' }).click();
  await expect(page.locator('h1')).toContainText('603 - S109 Kafirun Start => 604 - S112 Ikhlas Start');
  await page.getByRole('button', { name: 'Save' }).click();
  await expect(page).toHaveURL('http://localhost:5001/');
